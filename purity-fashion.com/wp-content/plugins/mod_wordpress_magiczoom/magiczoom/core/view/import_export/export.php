<?php
    function WordPress_MagicZoom_wp_export($set="all", $core_sett, $constructor_sett) {
        ...
        $license = magictoolbox_WordPress_MagicZoom_get_data_from_db();
        if ($license) {
        ...
        $scroll_license = magictoolbox_WordPress_MagicZoom_get_data_from_db('WordPress_MagicZoom_magicscroll');
        if ($scroll_license) {
        ...
        $filename = $sitename . 'wordpress.magiczoom.' . date( 'Y-m-d' ) . '.xml';
...
if ($constructor_settings) { ?>
    ...
    <?php foreach ($value as $key2 => $value2) { ?>
        <<?php echo $key2; ?>><?php echo ('options' != $key2 && 'additional_options' != $key2) ? $value2 : '<![CDATA['.$value2.']]>'; ?></<?php echo $key2; ?>>
    <?php } ?>
    </<?php echo $tool; ?>>
    <?php } ?>
</constructor>
