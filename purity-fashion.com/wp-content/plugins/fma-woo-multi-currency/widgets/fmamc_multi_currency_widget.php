...
class FMAMC_Multi_Currency extends WP_Widget {
	...
	function __construct() {
		...
		$this->WP_Widget( $this->widget_id, $this->widget_name, $widget_data );
	...
	function widget( $args, $data ) { 
		...
		$exclude = array_map( 'intval', explode( ',', $data['exclude'] ) );
		$order = $data['orderby'] == 'currency_name' ? 'asc' : 'desc';

		$currencies = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM ".$wpdb->prefix."fma_currency WHERE currency_status = 'published' ORDER BY currency_name", ARRAY_A));
